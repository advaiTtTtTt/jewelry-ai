...
# Try torchmcubes first; fall back to scikit-image marching cubes
try:
...
def _skimage_marching_cubes(vol: torch.Tensor, isolevel: float):
    """Fallback marching cubes using scikit-image (CPU-only)."""
    from skimage.measure import marching_cubes as sk_mc
    vol_np = vol.detach().cpu().numpy()
    verts, faces, _, _ = sk_mc(vol_np, level=isolevel)
...
class MarchingCubeHelper(IsosurfaceHelper):
    def __init__(self, resolution: int) -> None:
        ...
        self.mc_func: Callable = _torchmcubes_mc if _HAS_TORCHMCUBES else _skimage_marching_cubes
